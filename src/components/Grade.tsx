...
export function Grade({grade, date}: Props) {
  const today = dayjs().format('DD.MM');
  return (
    <div className={twMerge('py-2 pl-3 pr-5 rounded-lg border-2', grade >= 5 ? 'bg-5-main/10 border-5-main' : grade >= 4 ? 'bg-4-main/10 border-4-main' : grade >= 3 ? 'bg-3-main/10 border-3-main' : 'bg-2-main/10 border-2-main', date !== today ? 'border-transparent' : '')}>
